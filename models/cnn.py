...
from tensorflow.keras.layers import Input, Concatenate  # Merge 改成 Concatenate
...
from tensorflow.keras.layers import ELU, BatchNormalization
...
class ConvNN(object):
    ...
    def setup(self,X_train_shape):
        ...
        # 舊 API: normal2 = BatchNormalization(axis=1, mode=0, name='normal2')(reshape1)
        ...
        # 舊 API: normal3 = BatchNormalization(axis=1, mode=0, name='normal3')(pool2)
        ...
        # 舊 API: self.model = Model(input=inputs, output=last)
        self.model = Model(inputs=inputs, outputs=last)

        adam = Adam(learning_rate=5e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    ...
    def fit(self,X_train,Y_train,X_val=None, y_val=None):
        ...
        y_val = to_categorical(y_val, self.nb_classes) if y_val is not None else None
